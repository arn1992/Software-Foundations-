...
rewrite H.
symmetry.
Check rev_involutive. apply  rev_involutive. 
Qed.
(** [] *)
...
+ discriminate H1.
+ Check f_equal.  apply f_equal. apply IHn'.  simpl in H1. Check plus_n_Sm.
...
Proof.
  ...
  assert (H' : m = n). { Check eqb_true. apply eqb_true.  apply H. }
  rewrite H'. reflexivity.
Qed.
...
Check mult_assoc.
  ...
  { Check mult_comm. rewrite mult_comm. apply mult_assoc. }
...
Proof.
  ...
  (* Now we have the same state as at the point where we got
     ...
     make progress. *)
    - (* e3 = true *) Check eqb_true. apply eqb_true in Heqe3.
