...
Restart.
intros x y. unfold beq_string. destruct (string_dec x y).
- split. intros H1. apply e. intros H2. reflexivity.
- split. 
+ intros H3. inversion H3.
+ intros H4. exfalso. destruct n. apply H4.
...
Proof.
 ...
 Check  not_true_iff_false. rewrite not_true_iff_false. reflexivity. 
Restart.
intros x y. unfold beq_string.  destruct (string_dec x y).
- split. intros H1. discriminate H1. intros H2. destruct H2. apply e.
- split. intros H3. apply n. intros H4. reflexivity.
Qed.
...
Proof.
  ...
  intros H. apply H. 
Restart.
intros x y. rewrite <- beq_string_false_iff. intros H1. apply H1.
Qed.
...
Lemma t_update_eq : forall A (m: total_map A) x v,
  (m & {x --> v}) x = v.
Proof.
intros A m x v. unfold t_update. rewrite <-beq_string_refl. reflexivity.
Qed.
(** [] *)
...
Theorem t_update_neq : forall (X:Type) v x1 x2
  ...
  (m & {x1 --> v}) x2 = m x2.
Proof.
  intros X v x1 x2 m. unfold t_update. intros H1. rewrite false_beq_string . reflexivity.
apply H1.
Qed.
(** [] *)
...
Lemma t_update_shadow : forall A (m: total_map A) v1 v2 x,
    m & {x --> v1 ; x --> v2} = m & {x --> v2}.
Proof.
  intros A m v1 v2 x. Check @functional_extensionality. apply functional_extensionality.
intros x0. unfold t_update. destruct (beq_string x x0). 
- reflexivity.
- reflexivity.
Qed.
(** [] *)
...
Proof.
  intros x y. destruct (beq_string x y)eqn:H1.
- apply ReflectT. apply beq_string_true_iff. apply H1.
...
Theorem t_update_same : forall X x (m : total_map X),
    m & { x --> m x } = m.
  Proof.
  Check beq_stringP. intros X x m. apply functional_extensionality. intros x0. 
unfold t_update. Check beq_stringP. destruct (beq_stringP x x0).
...
Theorem t_update_permute : forall (X:Type) v1 v2 x1 x2
  ...
  =  m & { x1 --> v1 ; x2 --> v2 }.
Proof.
  intros X v1 v2 x1 x2 m. intros H1. apply functional_extensionality. intros x.
unfold t_update. destruct (beq_stringP x1 x).
- destruct (beq_stringP x2 x).
+ exfalso. unfold not in H1. apply H1. rewrite e. rewrite e0. reflexivity.
+ reflexivity.
- destruct (beq_stringP x2 x).
+ reflexivity.
+ reflexivity.
...
Lemma update_eq : forall A (m: partial_map A) x v,
    (m & {{ x --> v }}) x = Some v.
Proof.
  intros. unfold update. Check t_update_eq. rewrite t_update_eq.
  reflexivity.
Qed.
...
Theorem update_same : forall X v x (m : partial_map X),
  ...
  m & {{x --> v}} = m.
Proof.
  intros X v x m H. unfold update. rewrite <- H. Check t_update_same.
  apply t_update_same.
Qed.
